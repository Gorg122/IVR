...
function eme_insert_mailing($mailing_name, $planned_on, $subject, $body, $replytoemail, $replytoname, $mail_text_html) {
	global $wpdb;
...
function eme_remove_mail_from_queue($id) {
	global $wpdb;
	$mqueue_table = $wpdb->prefix.MQUEUE_TBNAME;
	$sql = "DELETE FROM $mqueue_table WHERE id=$id";
	return $wpdb->query($sql);
}
...
function eme_send_queued() {
	...
	$mailing_ids_seen=array();
	foreach ($mails as $mail) {
		$body=$mail['body'];
		if ($eme_rsvp_send_html) {
			$track_url=eme_tracker_url($mail['random_id']);
			$body.="<img src='$track_url' alt='' />";
		}
		...
		$res = eme_send_mail($mail['subject'],$body, $mail['receiveremail'], $mail['receivername'], $mail['replytoemail'], $mail['replytoname']);
		if ($mail['mailing_id']) {
			$mailing_ids_seen[]=$mail['mailing_id'];
			if ($res) {
			...
			// no mailing id, so we remove the mail from the queue after it has been treated
			eme_remove_mail_from_queue($mail['id']);
		}
		// be nice, sleep for half a second
		usleep(500000);
	}

	// now that we handled the mails, let's update the states of the mailings
	foreach ($mailing_ids_seen as $mailing_id) {
		eme_mark_ongoing_mailing($mailing_id);
	}
	...
	foreach ($ongoing_mailings as $mailing_id) {
		if (eme_count_mails_to_sent($mailing_id)==0) {
			eme_mark_completed_mailing($mailing_id);
		}
	}
}
...
function eme_mark_planned_mailing($mailing_id) {
	global $wpdb;
	$mailings_table = $wpdb->prefix.MAILINGS_TBNAME;
	$sql = $wpdb->prepare("UPDATE $mailings_table set status='planned' where id=%d",$mailing_id);
        $wpdb->query($sql);
}
function eme_mark_ongoing_mailing($mailing_id) {
	global $wpdb;
	$mailings_table = $wpdb->prefix.MAILINGS_TBNAME;
	$sql = $wpdb->prepare("UPDATE $mailings_table set status='ongoing' where id=%d",$mailing_id);
        $wpdb->query($sql);
}
function eme_mark_completed_mailing($mailing_id) {
	global $wpdb;
	$stats=serialize(eme_get_mailing_stats($mailing_id));
	$mailings_table = $wpdb->prefix.MAILINGS_TBNAME;
	$sql = $wpdb->prepare("UPDATE $mailings_table set status='completed', stats=%s where id=%d",$stats,$mailing_id);
        $wpdb->query($sql);
}
...
function eme_get_mailing_stats($mailing_id=0) {
	...
	$res=array('planned'=>0,'sent'=>0,'failed'=>0,'cancelled'=>0,'total_read_count'=>0);
...
function eme_mail_track($random_id) {
	...
	if (!empty($random_id)) {
		...
		// update the queue table with when the mail was read (only if not already done)
		if ($queued_mail['read_datetime']=='0000-00-00 00:00:00') {
			$sql = $wpdb->prepare("UPDATE $table SET read_datetime=%s, read_count=read_count+1 WHERE id = %d",$now, $queued_mail['id']);
			$wpdb->query($sql);
			...
			$sql = $wpdb->prepare("UPDATE $table SET read_count=read_count+1 WHERE id = %d",$queued_mail['id']);
			$wpdb->query($sql);
			$sql = $wpdb->prepare("UPDATE $mailings_table SET total_read_count=total_read_count+1 WHERE id = %d",$queued_mail['mailing_id']);
			$wpdb->query($sql);
		}

		// and then return a transparant image of 1x1
		$image = file_get_contents('images/1x1.gif');
...
function eme_mailingreport_list() {
   ...
   foreach ($rows as $item) {
	 ...
	 if ($item['status']>0) {
		 ...
		 if ($item['read_datetime'] != '0000-00-00 00:00:00') {
			 $localized_datetime = eme_localized_date($item['read_datetime']).' '.eme_localized_time($item['read_datetime']);
			 $record['read_datetime']=$localized_datetime; 
			 // to account for older mailings
			 if ($record['read_count']==0) $record['read_count']=1;
		 } else {
			 $record['read_datetime']=''; 
		 }
		 if ($item['person_id']>0 || $item['member_id']>0)
...
function eme_send_mails_ajax_actions($action) {
   ...
   if ($action == 'genericmail' || $action == 'previewmail') {
	   ...
	   elseif (isset($_POST['generic_subject_template']) && intval($_POST['generic_subject_template'])>0)
		   $mail_subject = eme_get_template_format(intval($_POST['generic_subject_template']),0);
	   else
		   $mail_subject = "";
	   ...
	   elseif (isset($_POST['generic_message_template']) && intval($_POST['generic_message_template'])>0)
		   $mail_message = eme_get_template_format(intval($_POST['generic_message_template']),0);
	   else
		   $mail_message = "";
	   ...
	   if (empty($contact_email)) {
		   $ajaxResult['htmlmessage'] = "<div id='message' class='error' style='background-color: rgb(255, 251, 204);'><p>".__('No contact email defined, bailing out','events-made-easy')."</p></div>";
		   $ajaxResult['result'] = "ERROR";
		   echo json_encode($ajaxResult);
		   return;
	   }
	   ...
	   if ($action=='previewmail') {
		   ...
		   if (isset($_POST['eme_send_all_people'])) {
			   ...
			   if (isset($_POST['eme_genericmail_send_persons']) && !empty($_POST['eme_genericmail_send_persons']) && eme_array_integers($_POST['eme_genericmail_send_persons'])) {
			   	$person_ids = $_POST['eme_genericmail_send_persons'];
			   }
			   ...
			   if (isset($_POST['eme_genericmail_send_peoplegroups']) && !empty($_POST['eme_genericmail_send_peoplegroups']) && eme_array_integers($_POST['eme_genericmail_send_peoplegroups'])) {
			   	$group_ids = join(',',$_POST['eme_genericmail_send_peoplegroups']);
			   	$person_ids = array_unique($person_ids + eme_get_groups_person_ids($group_ids));
			   }
			   if (isset($_POST['eme_genericmail_send_membergroups']) && !empty($_POST['eme_genericmail_send_membergroups']) && eme_array_integers($_POST['eme_genericmail_send_membergroups'])) {
			   	$group_ids = join(',',$_POST['eme_genericmail_send_membergroups']);
			   	$member_ids = array_unique($member_ids + eme_get_groups_member_ids($group_ids));
			   }
			   if (isset($_POST['eme_send_memberships']) && !empty($_POST['eme_send_memberships']) && eme_array_integers($_POST['eme_send_memberships'])) {
			   	...
			   	$member_ids = array_unique($member_ids + eme_get_memberships_member_ids($membership_ids));
			   }
		   }
		   if ($queue) {
			   $mailing_name = $_POST['mailing_name'];
			   $mailing_datetime = $_POST['genericmail_actualstartdate'];
			   ...
			   if ($mail_count==0) {
				   ...
				   $mailing_id = eme_insert_mailing($mailing_name,$mailing_datetime, $mail_subject, $mail_message, $contact_email, $contact_name, $mail_text_html);
			   }
		   }
	   }
	   $not_sent=array();
	   ...
	   foreach ( $person_ids as $person_id ) {
		   ...
		   if (is_email($person['email'])) {
			   ...
			   if (!array_key_exists($person['email'],$emails_handled)) {
	   ...
	   if (!$mail_problems) {
		   ...
		   if ($queue) {
			   $ajaxResult['htmlmessage'] = "<div id='message' class='updated' style='background-color: rgb(255, 251, 204);'><p>".__('The mailing has been put on the queue, but not all persons will receive it.','events-made-easy')."</p></div>";
			   if (!empty($not_sent)) {
				   $ajaxResult['htmlmessage'] .= "<div id='message' class='error' style='background-color: rgb(255, 251, 204);'><p>".__('The following persons will not receive the mail:','events-made-easy')." ".join(', ',$not_sent)."</p></div>";
			   }
		   } else {
			   $ajaxResult['htmlmessage'] = "<div id='message' class='error' style='background-color: rgb(255, 251, 204);'><p>".__('There were some problems while sending mail.','events-made-easy')."</p></div>";
			   if (!empty($not_sent)) {
   ...
   if ($action == 'eventmail') {
	   ...
	   elseif (isset($_POST ['event_subject_template']) && intval($_POST ['event_subject_template'])>0)
		   $mail_subject = eme_get_template_format(intval($_POST ['event_subject_template']),0);
	   else
		   $mail_subject = "";
	   ...
	   elseif (isset($_POST ['event_message_template']) && intval($_POST ['event_message_template'])>0)
		   $mail_message = eme_get_template_format(intval($_POST ['event_message_template']),0);
	   else
		   $mail_message = "";
	   ...
	   if ($queue) {
		   $mailing_name = $_POST['mailing_name'];
		   $mailing_datetime = $_POST['eventmail_actualstartdate'];
	   }
	   ...
	   foreach ($event_ids as $event_id) {
		   ...
		   if (current_user_can( get_option('eme_cap_send_other_mails')) ||
			   ...
			   if ($eme_mail_type == 'attendees') {
				   ...
				   $mail_count=count($attendees);
				   if ($queue && $mail_count >0)
					   $mailing_id = eme_insert_mailing($mailing_name,$mailing_datetime, $mail_subject, $mail_message, $contact_email, $contact_name, $mail_text_html);
				   foreach ( $attendees as $attendee ) {
					   ...
					   if ($mailing_id) {
						   $mail_res = eme_queue_mail($tmp_subject,$tmp_message, $attendee['email'], $person_name, $contact_email, $contact_name, $mailing_id, $person_id);
					   } else {
			   ...
			   } elseif ($eme_mail_type == 'all_people' || $eme_mail_type == 'people_and_groups' || $eme_mail_type == 'all_people_not_registered') {
				   if ($eme_mail_type == 'all_people') {
					   $person_ids = eme_get_massmail_person_ids();
				   } elseif ($eme_mail_type == 'all_people_not_registered') {
					   $person_ids = eme_get_massmail_person_ids($event_id);
				   } elseif ($eme_mail_type == 'people_and_groups') {
					   $person_ids = array();  
					   if (isset($_POST['eme_eventmail_send_persons']) && !empty($_POST['eme_eventmail_send_persons']) && eme_array_integers($_POST['eme_eventmail_send_persons'])) {
						   $person_ids = $_POST['eme_eventmail_send_persons'];
					   }
					   if (isset($_POST['eme_eventmail_send_groups']) && !empty($_POST['eme_eventmail_send_groups']) && eme_array_integers($_POST['eme_eventmail_send_groups'])) {
						   $group_ids = join(',',$_POST['eme_eventmail_send_groups']);
						   $person_ids = array_unique($person_ids + eme_get_groups_person_ids($group_ids));
					   }
				   }
				   $mail_count=count($person_ids);
				   if ($queue && $mail_count >0)
					   $mailing_id = eme_insert_mailing($mailing_name,$mailing_datetime, $mail_subject, $mail_message, $contact_email, $contact_name, $mail_text_html);
				   foreach ( $person_ids as $person_id ) {
					   $person = eme_get_person($person_id);
					   ...
					   $tmp_subject = eme_replace_people_placeholders($tmp_subject, $person, "text",0,$person['lang']);
					   $tmp_message = eme_replace_people_placeholders($tmp_message, $person, $mail_text_html,0,$person['lang']);
					   $person_name=$person['lastname'].' '.$person['firstname'];
				   ...
				   foreach ( $wp_users as $wp_user ) {
					   if ($mailing_id) {
						   $mail_res = eme_queue_mail($tmp_subject,$tmp_message, $wp_user->user_email, $wp_user->display_name, $contact_email, $contact_name, $mailing_id);
					   } else {
						   $mail_res=eme_send_mail($tmp_subject,$tmp_message, $wp_user->user_email, $wp_user->display_name, $contact_email, $contact_name);
						   if (!$mail_res) $not_sent[]=$wp_user->display_name;
					   }
					   if (!$mail_res) $mail_problems=true;
				   }
				   ...
				   $mail_count=count($wp_users)-count($attendee_wp_ids);
				   if ($queue && $mail_count >0)
					   $mailing_id = eme_insert_mailing($mailing_name,$mailing_datetime, $mail_subject, $mail_message, $contact_email, $contact_name, $mail_text_html);
				   $tmp_subject = eme_replace_placeholders($mail_subject, $event, "text");
				   $tmp_message = eme_replace_placeholders($mail_message, $event, $mail_text_html);
	   ...
	   if (!$mail_problems) {
		   ...
		   if ($mail_access_problems) {
			   ...
			   $ajaxResult['htmlmessage'] = "<div id='message' class='error' style='background-color: rgb(255, 251, 204);'><p>".__('There were some problems while sending mail.','events-made-easy')."</p></div>";
			   if (!empty($not_sent)) {
				   $ajaxResult['htmlmessage'] .= "<div class='error' style='background-color: rgb(255, 251, 204);'><p>".__('Mail to the following persons has not been sent:','events-made-easy')." ".join(', ',$not_sent)."</p></div>";
			   }
		   }
		   $ajaxResult['result'] = "ERROR";
	   }
...
<?php
   if (!$eme_queue_mails) {
?>
  <div style='background-color: rgb(255, 251, 204);'><p>
<?php
...
?>
   ...
   <div id='send_event_mail_div'>
      ...
      <td><?php _e('Send to a number of people','events-made-easy'); ?></td>
      <td><?php echo eme_ui_multiselect($person_ids,'eme_eventmail_send_persons',$mygroups,5,0,'eme_select2_people_class'); ?></td>
      </tr>
      <tr id="eme_groups_row">
      <td width='20%'><?php _e('Send to a number of groups','events-made-easy'); ?></td>
      <td><?php echo eme_ui_multiselect_key_value('','eme_eventmail_send_groups',$peoplegroups,'group_id','name','', 5,0,'eme_select2_groups_class'); ?></td>
      </tr>
      </table>
      ...
      <input type="text" name="event_mail_subject" id="event_mail_subject" value="" />
	   </p></div>
	   <div class="form-field form-required"><p>
      ...
      ?>
	   ...
	   <div id='div_mailing_definition'>
		...
		<input type='hidden' name='eventmail_actualstartdate' id='eventmail_actualstartdate' value='<?php echo $this_datetime; ?>'>
		<input type='text' name='eventmail_startdate' id='eventmail_startdate' value='<?php echo $localized_datetime; ?>' style="background: #FCFFAA;" readonly="readonly">
		</p>
	   </div>
	   <div>
		<p>
  ...
  <div id="tab-genericmails">
   ...
   <?php _e ( "Use the below form to send a generic mail. Don't forget to use the #_UNSUB_URL for unsubscribe possibility.", 'events-made-easy');
	   if (!$eme_queue_mails) {
		   print "<br /><div style='background-color: rgb(255, 251, 204);'><p>";
   ...
   <form id='send_generic_mail' name='send_generic_mail' action="#" method="post" onsubmit="return false;">
	   <div class="form-field form-required">
		...
		<tr><td width='20%'><?php _e('Send to a number of people','events-made-easy'); ?></td>
                <td><?php echo eme_ui_multiselect($person_ids,'eme_genericmail_send_persons',$mygroups,5,0,'eme_select2_people_class'); ?></td></tr>
		<tr><td width='20%'><?php _e('Send to a number of groups','events-made-easy'); ?></td>
		<td><?php echo eme_ui_multiselect_key_value('','eme_genericmail_send_peoplegroups',$peoplegroups,'group_id','name','', 5,0,'eme_select2_groups_class'); ?></td></tr>
		<tr><td width='20%'><?php _e('Send to a number of members','events-made-easy'); ?></td>
                <td><?php echo eme_ui_multiselect($member_ids,'eme_send_members',$mymembergroups,5,0,'eme_select2_members_class'); ?></td></tr>
		<tr><td width='20%'><?php _e('Send to a number of member groups','events-made-easy'); ?></td>
		<td><?php echo eme_ui_multiselect_key_value('','eme_genericmail_send_membergroups',$membergroups,'group_id','name','', 5,0,'eme_select2_groups_class'); ?></td></tr>
		<tr><td width='20%'><?php _e('Send to active members belonging to','events-made-easy'); ?></td>
	   ...
	   <div id='div_mailing_definition'>
		...
		<b><?php _e('Set mailing name and start date and time','events-made-easy'); ?></b><br />
                <?php _e('Mailing name: ','events-made-easy'); ?> <input type='text' name='mailing_name' id='mailing_name' value='' required='required'><br />
                <?php _e('Start date and time: ','events-made-easy'); ?>
		<input type='hidden' name='genericmail_actualstartdate' id='genericmail_actualstartdate' value='<?php echo $this_datetime; ?>'>
		<input type='text' name='genericmail_startdate' id='genericmail_startdate' value='<?php echo $localized_datetime; ?>' style="background: #FCFFAA;" readonly="readonly">
		</p>
	   </div>
	   <div>
		<p>
  ...
  <div id="tab-testmail">
   ...
   <?php _e('Enter the recipient','events-made-easy'); ?>
...
function eme_ajax_mailings_div() {
   ...
   foreach ($mailings as $mailing) {
	   ...
	   if ($mailing['status']=='cancelled') {
		   ...
		   $extra=sprintf(__('%d mails delivered, %d mails failed, %d mails cancelled','events-made-easy'),$stats['sent'],$stats['failed'],$stats['cancelled']);
		   $action = "<a onclick='return areyousure(\"$areyousure\");' href='".wp_nonce_url(admin_url("admin.php?page=eme-send-mails&amp;eme_admin_action=delete_mailing&amp;id=".$id),'eme_mailings','eme_admin_nonce')."'>".__('Delete','events-made-easy')."</a> <a onclick='return areyousure(\"$areyousure\");' href='".wp_nonce_url(admin_url("admin.php?page=eme-send-mails&amp;eme_admin_action=archive_mailing&amp;id=".$id),'eme_mailings','eme_admin_nonce')."'>".__('Archive','events-made-easy')."</a>";
		   ...
		   $stats = eme_get_mailing_stats($id);
		   $extra=sprintf(__('%d mails delivered, %d mails failed, %d mails left','events-made-easy'),$stats['sent'],$stats['failed'],$stats['planned']);
		   $action = "<a onclick='return areyousure(\"$areyousure\");' href='".wp_nonce_url(admin_url("admin.php?page=eme-send-mails&amp;eme_admin_action=cancel_mailing&amp;id=".$id),'eme_mailings','eme_admin_nonce')."'>".__('Cancel','events-made-easy')."</a>";
	   } elseif ($mailing['status']=='archived') {
		   $status=__('Archived','events-made-easy');
		   $stats=unserialize($mailing['stats']);
		   $extra=sprintf(__('%d mails delivered, %d mails failed, %d mails cancelled','events-made-easy'),$stats['sent'],$stats['failed'],$stats['cancelled']);
		   ...
		   $stats=unserialize($mailing['stats']);
		   $extra=sprintf(__('%d mails delivered, %d mails failed','events-made-easy'),$stats['sent'],$stats['failed']);
		   $action = "<a onclick='return areyousure(\"$areyousure\");' href='".wp_nonce_url(admin_url("admin.php?page=eme-send-mails&amp;eme_admin_action=delete_mailing&amp;id=".$id),'eme_mailings','eme_admin_nonce')."'>".__('Delete','events-made-easy')."</a> <a onclick='return areyousure(\"$areyousure\");' href='".wp_nonce_url(admin_url("admin.php?page=eme-send-mails&amp;eme_admin_action=archive_mailing&amp;id=".$id),'eme_mailings','eme_admin_nonce')."'>".__('Archive','events-made-easy')."</a>";
	   }
