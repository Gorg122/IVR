...
function eme_gdpr_shortcode() {
	...
	if (isset($_POST['eme_action']) && $_POST['eme_action']=='eme_gdpr_mail' && isset($_POST['eme_email'])) {
		...
		$output= '<div id="message" class="info" style="background-color: rgb(255, 251, 204);">';
		$output .= __('Thank you for your request, an email will be sent with further info','events-made-easy');
		$output .= '</div>';
	} else {
		$output ="
   <form id='eme_gdpr_form' method='post' action='#'>
   <input type='hidden' name='eme_action' value='eme_gdpr_mail' />
   <input type='email' name='eme_email' value='' placeholder='".__('E-mail','events-made-easy')."' />
...
function eme_gdpr_show($email) {
	print eme_translate(get_option('eme_gdpr_page_header'));
	if (eme_count_persons_by_email($email)>0) {
	...
	print eme_translate(get_option('eme_gdpr_page_footer'));
}
...
function eme_replace_people_placeholders($format, $person, $target="html", $lang='', $do_shortcode=1, $do_translate=1) {
...
function eme_manage_people_layout($message="") {
   ...
   <?php echo eme_ui_multiselect_key_value('','search_groups',$groups,'group_id','name','', 5,0,'eme_select2_people_groups_class'); ?>
   <?php
...
function eme_get_person_by_name_and_email($lastname, $firstname, $email) {
   ...
   if (!$res && get_option('eme_rsvp_check_without_accents')) {
	   if (!empty($firstname))
		   $sql = $wpdb->prepare("SELECT * FROM $people_table WHERE lastname = %s AND firstname = %s AND email = %s",remove_accents($lastname),remove_accents($firstname),$email);
	   else
		   $sql = $wpdb->prepare("SELECT * FROM $people_table WHERE lastname = %s AND email = %s",remove_accents($lastname),$email);
	   $res = $wpdb->get_row($sql, ARRAY_A);
   }
   return $res;
}
...
function eme_get_massmail_person_ids($except_event_id=0) {
   global $wpdb; 
   $people_table = $wpdb->prefix.PEOPLE_TBNAME;
   $bookings_table = $wpdb->prefix.BOOKINGS_TBNAME;
...
function eme_get_groups_person_ids($group_ids) {
   ...
   if (!empty($static_groupids)) {
	   $sql = "SELECT people.person_id FROM $people_table AS people LEFT JOIN $usergroups_table as ugroups ON people.person_id=ugroups.person_id WHERE ugroups.group_id IN ($static_groupids) GROUP BY people.email";
...
function eme_get_memberships_member_ids($membership_ids) {
   ...
   $sql = "SELECT members.member_id FROM $people_table AS people LEFT JOIN $members_table as members ON people.person_id=members.person_id WHERE members.state=1 AND members.membership_id IN ($membership_ids) GROUP BY people.email";
   return $wpdb->get_col($sql);
}
...
function eme_ajax_people_list($dynamic_groupname='') {
   ...
   $search_groups = isset($_REQUEST['search_groups']) ? join(',',$_REQUEST['search_groups']) : '';
   $where ='';
   $where_arr = array();
   ...
   if (!empty($search_groups)) {
      $where_arr[] = "group_id IN ($search_groups)";
   }
   if ($where_arr)
   ...
   if (empty($formfields)) {
	   $count_sql = "SELECT COUNT(*) FROM $people_table as people $usergroup_join
		   $where";
	   $stored_sql = "FROM $people_table as people $usergroup_join
		   $where";
...
function eme_ajax_groups_list() {
   ...
   foreach ($groups as $group) {
         ...
         $record['name'] = "<a href='".wp_nonce_url(admin_url("admin.php?page=eme-groups&amp;eme_admin_action=edit_group&amp;group_id=".$group['group_id']),'eme_people','eme_admin_nonce')."' title='".__('Edit group','events-made-easy')."'>".eme_esc_html($group['name'])."</a>";
