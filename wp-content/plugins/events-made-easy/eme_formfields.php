...
function eme_replace_cancelformfields_placeholders ($event) {
   ...
   if (preg_match('/#_SUBMIT\{.+\}/s', $format)) {
      $format = preg_replace('/#_SUBMIT\{(.+?)\}/s', "<img id='loading_gif' src='".EME_PLUGIN_URL."images/spinner.gif' style='display:none;'><input name='eme_submit_button' class='eme_submit_button' type='submit' value='".eme_trans_esc_html('$1')."' />" ,$format );
...
function eme_replace_rsvp_formfields_placeholders ($event,$booking,$format="",$is_multibooking=0,$return_postfieldname=0) {
   ...
   if (preg_match('/#_SUBMIT\{.+\}/s', $format)) {
      if ($editing_booking_from_backend)
         $format = preg_replace('/#_SUBMIT\{(.+?)\}/s', "<img id='loading_gif' src='".EME_PLUGIN_URL."images/spinner.gif' style='display:none;'><input name='eme_submit_button' class='eme_submit_button' type='submit' value='".__('Update booking','events-made-easy')."' />" ,$format );
      else
         $format = preg_replace('/#_SUBMIT\{(.+?)\}/s', "<img id='loading_gif' src='".EME_PLUGIN_URL."images/spinner.gif' style='display:none;'><input name='eme_submit_button' class='eme_submit_button' type='submit' value='".eme_trans_esc_html('$1')."' />" ,$format );
      if (!$is_multibooking)
         $required_fields_count++;
   ...
   if (eme_is_multi($event['price'])) {
	 ...
	 if ($eme_captcha_for_booking)
         ...
         return "<div id='message' class='eme-rsvp-message-error'>$res</div>";
      }
...
function eme_dyndata_adminform($eme_data,$templates_array) {
   ...
   <div id="div_dyndata">
      ...
      <table class="eme_dyndata">
         ...
         <tfoot>
            <tr>
               <td colspan="8"><a href="#" id="eme_dyndata_add_tag"><?php _e('Add new condition','events-made-easy'); ?></a></td>
            </tr>
         </tfoot>
      </table>
      <p class='eme_smaller'>
