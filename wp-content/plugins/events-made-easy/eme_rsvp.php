...
function eme_add_multibooking_form($event_ids,$template_id_header=0,$template_id_entry=0,$template_id_footer=0,$eme_register_empty_seats=0,$is_multibooking=1,$only_if_not_registered=0) {
   ...
   foreach ($events as $event) {
	   ...
	   if (!$seats_available) {
		   if (!$is_multibooking)
			   $form_html.="<div class='eme-rsvp-message-error'>".__('Bookings no longer possible: no seats available anymore', 'events-made-easy')."</div>";
	   } else {
...
function eme_attendees_csv_report($scope,$category,$notcategory,$event_template_id,$attend_template_id) {
   $events = eme_get_events("scope=$scope&category=$category&notcategory=$notcategory");
   $attend_format = eme_get_template_format($template_id,0);
...
function eme_delete_booking_form($event_id) {
   ...
   if ($event_rsvp_datetime->lessThan($eme_date_obj_now->copy()->modifyDays($event['rsvp_number_days'])->modifyHours($event['rsvp_number_hours'])) ||
      ...
      $form_html="<div class='eme-rsvp-message-error'>".$no_longer_allowed."</div>";
   } else {
	   $current_userid=get_current_user_id();
...
function eme_add_bookings_ajax() {
	 ...
	 if ($is_multibooking) {
      ...
      if ($payment_id && eme_event_can_pay_online($events[0])) {
	    ...
	    eme_update_payment_paid($payment_id,0);
...
function eme_multibook_seats($events, $send_mail, $format, $is_multibooking=1) {
   ...
   foreach ($events as $event) {
	 ...
	 if (!$is_multibooking)
      ...
      if (!$is_admin && ($event['event_properties']['person_only_once'] || $event['event_properties']['email_only_once'])) {
	 ...
	 if ($event['event_properties']['person_only_once']) {
	      ...
	      if ($already_registered) {
		      $result .= __('This person has already registered.','events-made-easy');
		      continue;
	      }
	 } elseif ($event['event_properties']['email_only_once']) {
              $already_registered=0;
	      if (!isset($_POST['email']) || !is_email($_POST['email'])) {
		      $result .= __('No valid email provided.','events-made-easy');
		      continue;
	      }
	      ...
	      if ($already_registered) {
		      $result .= __('This email has already registered.','events-made-easy');
		      continue;
	      }
	 }
      }
...
function eme_email_rsvp_booking($booking,$action, $is_multibooking=0, $queue=0) {
   ...
   elseif ($event['event_properties']['event_respondent_email_body_tpl']>0)
      $confirmed_body = eme_get_template_format($event['event_properties']['event_respondent_email_body_tpl'],0);
   else
   ...
   elseif ($event['event_properties']['event_registration_pending_email_body_tpl']>0)
      $pending_body = eme_get_template_format($event['event_properties']['event_registration_pending_email_body_tpl'],0);
   else
      $pending_body = get_option('eme_registration_pending_email_body' );
   $pending_reminder_subject=eme_get_template_format(intval(get_option('eme_cron_reminder_unpaid_subject')),0);
   $pending_reminder_body=eme_get_template_format(intval(get_option('eme_cron_reminder_unpaid_body')),0);
   ...
   elseif ($event['event_properties']['event_registration_denied_email_body_tpl']>0)
      $denied_body = eme_get_template_format($event['event_properties']['event_registration_denied_email_body_tpl'],0);
   else
   ...
   elseif ($event['event_properties']['event_registration_updated_email_body_tpl']>0)
      $updated_body = eme_get_template_format($event['event_properties']['event_registration_updated_email_body_tpl'],0);
   else
   ...
   elseif ($event['event_properties']['event_registration_cancelled_email_body_tpl']>0)
      $cancelled_body = eme_get_template_format($event['event_properties']['event_registration_cancelled_email_body_tpl'],0);
   else
   ...
   elseif ($event['event_properties']['event_registration_paid_email_body_tpl']>0)
      $paid_body = eme_get_template_format($event['event_properties']['event_registration_paid_email_body_tpl'],0);
   else
   ...
   elseif ($event['event_properties']['event_contactperson_email_body_tpl']>0)
      $contact_body = eme_get_template_format($event['event_properties']['event_contactperson_email_body_tpl'],0);
   else
...
function eme_registration_seats_page($pending=0) {
   ...
   if (isset($_GET['eme_admin_action']) && $_GET['eme_admin_action'] == "newRegistration" && isset($_GET['event_id'])) {
      ...
      if (get_option('eme_rsvp_admin_allow_overbooking')) {
	      $ret_string.="<div class='eme-rsvp-message-success'>".__('Be aware: the overbooking option is set.', 'events-made-easy')."</div>";
      }
