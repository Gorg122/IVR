...
function eme_init_membership_props($props) {
   ...
   $payment_gateways=array('use_paypal','use_2co','use_webmoney','use_fdgg','use_mollie','use_sagepay','use_worldpay','use_stripe','use_braintree','use_paymill','use_offline');
...
?>
   ...
   <td>
       ...
       <?php echo eme_ui_use_checkbox($membership['properties']['use_paymill'],"properties[use_paymill]",__( 'Paymill','events-made-easy')); ?><br />
...
function eme_manage_members_layout($message) {
   ...
   <?php
   echo eme_ui_multiselect_key_value('','search_membershipids',$memberships,'membership_id','name','',5,0,'eme_select2_memberships_class');
   ?>
   <?php
   echo eme_ui_multiselect('','search_status',$eme_member_status_array,5,0,'eme_select2_status_class');
   ?>
   ...
   echo eme_ui_multiselect_key_value('','search_customfieldids',$formfields,'field_id','field_name','',5,0,'eme_select2_customfieldids_class');
   ?>
   ...
   <div id="hint">
        <?php _e('Showing custom field values can be heavy on the database. If you want to show some field values, select these in the "Custom fields" select-box shown above.','events-made-easy'); ?>
   </div>
   </form>
...
function eme_email_member_booking($member, $action, $queue=0) {
   ...
   $member_new_body = eme_get_template_format($membership['properties']['new_body_format_tpl'],0);
   $member_extended_subject = $membership['properties']['extended_subject_format'];
   $member_extended_body = eme_get_template_format($membership['properties']['extended_body_format_tpl'],0);
   $member_updated_subject = $membership['properties']['updated_subject_format'];
   $member_updated_body = eme_get_template_format($membership['properties']['updated_body_format_tpl'],0);
   $member_paid_subject = $membership['properties']['paid_subject_format'];
   $member_paid_body = eme_get_template_format($membership['properties']['paid_body_format_tpl'],0);
   $member_stop_subject = $membership['properties']['stop_subject_format'];
   $member_stop_body = eme_get_template_format($membership['properties']['stop_body_format_tpl'],0);
   $member_reminder_subject = $membership['properties']['reminder_subject_format'];
   $member_reminder_body = eme_get_template_format($membership['properties']['reminder_body_format_tpl'],0);
   $contact_new_subject = $membership['properties']['contact_new_subject_format'];
   $contact_new_body = eme_get_template_format($membership['properties']['contact_new_body_format_tpl'],0);
   $contact_paid_subject = $membership['properties']['contact_paid_subject_format'];
   $contact_paid_body = eme_get_template_format($membership['properties']['contact_paid_body_format_tpl'],0);
   $contact_stop_subject = $membership['properties']['contact_stop_subject_format'];
   $contact_stop_body = eme_get_template_format($membership['properties']['contact_stop_body_format_tpl'],0);
   $contact_ipn_subject = $membership['properties']['contact_ipn_subject_format'];
   $contact_ipn_body = eme_get_template_format($membership['properties']['contact_ipn_body_format_tpl'],0);
...
function eme_replace_member_placeholders($format, $membership, $member, $target="html") {
   ...
   $do_shortcode=0;
   $do_translate=0;
   $format = eme_replace_membership_placeholders($format, $membership, $target, $lang, $do_shortcode, $do_translate);
   $format = eme_replace_people_placeholders($format, $person, $target, $lang, $do_shortcode, $do_translate);
   $format = eme_translate($format,$lang);
   return do_shortcode($format);
}

function eme_replace_membership_placeholders($format, $membership, $target="html", $lang='', $do_shortcode=1, $do_translate=1) {
	preg_match_all("/#_?[A-Za-z0-9_]+(\{.*?\})?(\{.*?\})?/", $format, $placeholders);
	usort($placeholders[0],'eme_sort_stringlenth');
...
function eme_ajax_members_list($dynamic_groupname='') {
   ...
   if (isset($_REQUEST['search_membershipids']) && !empty($_REQUEST['search_membershipids'])) {
	   $search_membershipids=join(',',$_REQUEST['search_membershipids']);
	   $where_arr[]="(members.membership_id IN ($search_membershipids))";
   }
   // search_status can be 0 too, for pending
   if (isset($_REQUEST['search_status']) && !empty($_REQUEST['search_status'])) {
	   $search_status=join(',',$_REQUEST['search_status']);
	   $where_arr[]="(members.status IN ($search_status))";
   }
   if (!empty($where_arr)) {
...
function eme_ajax_store_members_query() {
   check_ajax_referer('eme_members','eme_admin_nonce');
   if (isset($_POST['dynamicgroupname']) && !empty($_POST['dynamicgroupname'])) {
        eme_ajax_members_list($_POST['dynamicgroupname']);
        $jTableResult['htmlmessage'] = __('Dynamic group added','events-made-easy');
   } else {
        $jTableResult['htmlmessage'] = __('Please enter a name for the group','events-made-easy');
   }
   print json_encode($jTableResult);
   wp_die();
}
