   ...
   if (is_admin()) {
	 ...
	 // a discount can only be applied once
         ...
         if ($max_discounts==0 || $group_count<$max_discounts) {
            ...
            if ($res=eme_calc_booking_discount($discount,$booking,$do_update)) {
               $total_discount+=$res;
               $group_count++;
...
function eme_increase_discount_count($id,$usage=1) {
   global $wpdb;
   $table = $wpdb->prefix.DISCOUNTS_TBNAME;
...
function eme_calc_booking_discount($discount,$booking,$do_update) {
   ...
   if ($res && $do_update) {
	 // if we want discount usage per seat, we can only take into account the "free" discount codes
	 if ($discount['use_per_seat']) {
		 if ($discount['count']+$booking['booking_seats']>$discount['maxcount']) {
			 $discount_count_to_use=$discount['maxcount']-$discount['count'];
			 eme_increase_discount_count($discount['id'],$discount_count_to_use);
		 } else {
			 eme_increase_discount_count($discount['id'],$booking['booking_seats']);
		 }
	 } else {
...
function eme_calc_discount($discount,$booking,$coupon) {
   ...
   if  (($discount['strcase'] && strcmp($discount['coupon'],$coupon)===0) ||
	 ...
	 // type 4 is discount per seat, but if we also want discount usage per seat, we can only
